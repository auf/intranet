...
@NamedQueries({ @NamedQuery(name = "AufImplantation.findAll", query = "SELECT i FROM AufImplantation i WHERE i.active=1 AND i.status=1 ORDER BY i.name"), @NamedQuery(name = "AufImplantation.findAllCities", query = "SELECT DISTINCT(i.city) FROM AufImplantation i WHERE i.active=1 AND i.status=1 ORDER BY i.city"), @NamedQuery(name = "AufImplantation.findAllTypes", query = "SELECT DISTINCT(i.type) FROM AufImplantation i WHERE i.active=1 AND i.status=1 ORDER BY i.type"), })
public class AufImplantation {
	...
	@ManyToOne
	@JoinColumn(name = "zone_administrative_id")
